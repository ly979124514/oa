<template>
   <f7-page pull-to-refresh @ptr:refresh="onRefresh" @page:init="getData">
    ...
    <f7-list style="text-align:left">
      <f7-list-item v-for="item in show_items" 
                    :title="item.NOTICE_TITLE"
                    :after="item.NOTICE_PUBLISHTIME"
                    @click="$router.load({ url : '/notice_content?title='+item.NOTICE_TITLE})"
      ></f7-list-item> 
     </f7-list>
...
export default {
  ...
  methods: {
    getData: function() {
       this.$http.get('http://172.31.0.31:8081/cat/getNotice').then(function(response){
